...
// Comprobar si se ha proporcionado un ID de usuario
...
// Verificar que no se esté intentando editar al usuario actual
if($id == $_SESSION["id"]){
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($email_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        if(!empty($password)){
            ...
            $sql = "UPDATE usuarios SET username = ?, email = ?, password = ?, es_admin = ? WHERE id = ?";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, "sssis", $username, $email, $hashed_password, $es_admin, $id);
            $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        } else {
            // Actualizar sin cambiar contraseña
            $sql = "UPDATE usuarios SET username = ?, email = ?, es_admin = ? WHERE id = ?";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, "ssis", $username, $email, $es_admin, $id);
        }
    ...
    $sql = "SELECT username, email, es_admin FROM usuarios WHERE id = ?";
    ...
    if($stmt = mysqli_prepare($conn, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_stmt_num_rows($stmt) == 1){
                mysqli_stmt_bind_result($stmt, $username, $email, $es_admin);
                mysqli_stmt_fetch($stmt);
            } else{
                // El usuario no existe, redirigir
                header("location: usuarios.php");
                exit;
            }
        } else{
          ...
          <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $id; ?>" method="post">
            <div class="card-body">
              ...
              <div class="form-group">
                ...
                <input type="password" class="form-control <?php echo (!empty($password_err)) ? 'is-invalid' : ''; ?>" id="password" name="password" placeholder="Dejar en blanco para mantener la actual">
